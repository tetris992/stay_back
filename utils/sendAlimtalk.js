 ...
 * API 키를 받지 못한 상태라면 기본값을 그대로 두어 이후 체크할 수 있습니다.
 */
const authData = {
  apikey: process.env.ALIGO_API_KEY || 'YOUR_API_KEY',  // 환경변수를 사용하거나 기본값 설정
  userid: process.env.ALIGO_USER_ID || 'YOUR_USER_ID',  // 환경변수를 사용하거나 기본값 설정
};
...
export async function sendReservationConfirmation(reservation, hotelSettings) {
  ...
  // API 인증 정보가 설정되지 않은 경우(기본값일 경우)
  if (
    authData.apikey === 'YOUR_API_KEY' ||
    authData.userid === 'YOUR_USER_ID'
  ) {
    console.warn(
  ...
  const payload = {
    senderkey: process.env.ALIGO_SENDER_KEY || 'YOUR_SENDER_KEY', // 환경변수를 사용
    tpl_code: 'TX_8844', // 실제 템플릿 코드로 교체
  ...
  try {
    // aligoapi 모듈 사용 (동적 import 또는 사전에 설치된 모듈 사용)
