...
export const createOrUpdateReservationsExtension = async (req, res) => {
  ...
  logger.info('Received reservation data:', { siteName, reservations, hotelId });
  ...
  try {
    ...
    for (const reservation of reservations) {
      ...
      // OTA 예약: 원본 시간 문자열을 KST로 파싱 후 그대로 저장
      let checkIn = reservation.checkIn;
      let checkOut = reservation.checkOut || `${reservation.checkIn.split(' ')[0]} 11:00`;
      const reservationDate = reservation.reservationDate || format(new Date(), "yyyy-MM-dd'T'HH:mm:ss+09:00");

      const parsedCheckIn = await parseDate(checkIn, finalHotelId, true); // KST 문자열 반환
      const parsedCheckOut = await parseDate(checkOut, finalHotelId, false); // KST 문자열 반환
      if (!parsedCheckIn || !parsedCheckOut || new Date(parsedCheckIn) >= new Date(parsedCheckOut)) {
      ...
      // KST 문자열 그대로 사용 (UTC 변환 제거)
      checkIn = parsedCheckIn; // "yyyy-MM-dd'T'HH:mm:ss+09:00"
      checkOut = parsedCheckOut; // "yyyy-MM-dd'T'HH:mm:ss+09:00"
      ...
      const sanitizedPhoneNumber = sanitizePhoneNumber(reservation.phoneNumber || '');
      ...
      if (existingCanceled) {
        if (cancelled) {
          ...
          emitWebSocketEvent(req.app.get('io'), finalHotelId, 'reservationCreated', {
      ...
      if (existingReservation) {
        if (cancelled) {
          ...
          emitWebSocketEvent(req.app.get('io'), finalHotelId, 'reservationDeleted', {
            reservationId,
          });
        } else {
          if (availableOTAs.includes(siteName)) {
          ...
          if (updatedReservation) {
            emitWebSocketEvent(req.app.get('io'), finalHotelId, 'reservationUpdated', {
